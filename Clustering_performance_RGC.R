...
### Clustering performance for RGCs---------------------------------------------
## IPL-CSP gene sets---------------

RGC_obj <- readRDS("./Saved_objects/p5rgc_object_allgenes_finalcells.rds") #Load datasets
RGC_subset_IPLCSP <- readRDS("./Saved_objects/p5rgc_object_IPLCSP.rds")
...
available_genes <- genes_vector_TPlist[genes_vector_TPlist %in% rownames(RGC_obj)]
...
calculate_metrics_for_resolutions <- function(seu_obj, sample_name) {
  ...
  for (res in resolutions) {
    ...
    reference_labels <- seu_obj$predicted.id
...
csp_results <- calculate_metrics_for_resolutions(RGC_subset_IPLCSP, "csp_genes")
results_df <- rbind(results_df, csp_results)
seurat_objects[["csp_genes"]] <- RGC_subset_IPLCSP
...
for (i in 1:n_random_samples) {
  ...
  set.seed(i)
  genes_vector <- sample(availablegenes_notcsp, 1131)
  
  subset_seu <- subset(RGC_obj, features = genes_vector)
  ...
  subset_seu <- RunUMAP(subset_seu, reduction = "harmony", dims = 1:25, verbose = FALSE)
  subset_seu <- FindNeighbors(subset_seu, reduction = "harmony", dims = 1:25, verbose = FALSE)
...
DimPlot(seu_obj, reduction = "umap", group.by = "predicted.id", label = T, repel = T, label.size = 4) + NoLegend()

#Optional: save list of seurat objects
saveRDS(seurat_objects, "./Saved_objects/p5RGC_seurat_objects_IPLCSP_sets.rds", compress = "gzip")


## Module gene sets---------------

RGC_obj <- readRDS("./Saved_objects/p5rgc_object_allgenes_finalcells.rds") #Load datasets
RGC_subset_Mods <- readRDS("./Saved_objects/p5rgc_object_Mods.rds")

RGC_mods <- read.xlsx("./Data/RGC_modules.xlsx") #Load RGC module genes
genes_vector_Mods <- RGC_mods$gene_name
...
for (i in 1:n_random_samples) {
  ...
  set.seed(i)
  genes_vector <- sample(availablegenes_notcsp, 351)
...
#Optional: save list of seurat objects
saveRDS(seurat_objects, "./Saved_objects/p5RGC_seurat_objects_Module_sets.rds", compress = "gzip")
